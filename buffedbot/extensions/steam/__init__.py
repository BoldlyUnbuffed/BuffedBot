...
class Steam(commands.Cog, name='steam'):
    ...
    async def game(self, ctx, *message):
        ...
        async with ctx.typing():
            game = " ".join(message)
            results = await self.get_search_results(game)
            top_result = results[0]
            if top_result['name'].lower() != game.lower():
                return await ctx.reply(
                    f'*"{game}" not found. Did you mean "{top_result["name"]}"?*'
                )
            embed = game_to_discord_embed(
                await self.get_game(top_result['url'])
            )
            await ctx.reply(embed=embed)
